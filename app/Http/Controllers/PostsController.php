        ...
        if ($selectedPosts) {
            if ($action === 'trash') {
                Posts::whereIn('id', $selectedPosts)
                  ...
                  notify()->success(__('posts.bulk.deleted'));
            } elseif ($action === 'delete') {
                Posts::whereIn('id', $selectedPosts)->forceDelete();
                notify()->success(__('posts.bulk.force_deleted'));
            } elseif ($action === 'publish') {
                Posts::whereIn('id', $selectedPosts)
                    ->update(['status' => PostStatus::Published->value]);
               notify()->success(__('posts.bulk.published'));
            } elseif ($action === 'draft') {
                Posts::whereIn('id', $selectedPosts)
                    ->update(['status' => PostStatus::Draft->value]);
                 notify()->success(__('posts.bulk.drafted'));
            } elseif ($action === 'kembalikan') {
                Posts::whereIn('id', $selectedPosts)
                    ->restore(); // Mengembalikan dari soft delete
                Posts::whereIn('id', $selectedPosts)
                    ->update(['status' => PostStatus::Published->value]);
              notify()->success(__('posts.bulk.restored'));
            } else {
               notify()->error(__('posts.bulk.select_action'));
                return redirect()->back();
            }
    
            return redirect()->route('posts.index', ['status' => $request->query('status')]);
        } else {
          notify()->error(__('posts.bulk.select_post'));
            return redirect()->back();
        }
    }
